...
int main() {
    ...
    */
    ...
    // ------------------ WRITE CODE HERE ------------------
    char pid_path_name[64];
    snprintf(pid_path_name,sizeof(pid_path_name),"/proc/%d/ns/pid",child_pid);
    int pid_ns=open(pid_path_name, O_RDONLY);
    ...
    if (fork() == 0) {
        ...
        */
        ...
        // ------------------ WRITE CODE HERE ------------------
        char child_pid_path_name[64];
        snprintf(child_pid_path_name,sizeof(child_pid_path_name),"/proc/%d/ns/uts",child_pid);
        int child_pid_ns=open(child_pid_path_name, O_RDONLY);
        setns(child_pid_ns,  CLONE_NEWUTS );

        //setns(child_pid,  CLONE_NEWUTS | CLONE_NEWPID | SIGCHLD);
