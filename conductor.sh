...
run()
{
    ...
    # sudo chmod 755 $CONTAINERDIR/$NAME/rootfs
    
    # mount -t proc proc $CONTAINERDIR/$NAME/rootfs/proc
   
    # mkdir -p "$CONTAINERDIR/$NAME/rootfs/sysfs"
    # mount -t sysfs sysfs $CONTAINERDIR/$NAME/rootfs/sysfs
    # mkdir -p "$CONTAINERDIR/$NAME/rootfs/container-rootfs"
    # mount --bind /dev $CONTAINERDIR/$NAME/rootfs/container-rootfs
    

    # findmnt --target $CONTAINERDIR/$NAME/rootfs/container-rootfs
    # findmnt --target $CONTAINERDIR/$NAME/rootfs/proc
    # findmnt --target $CONTAINERDIR/$NAME/rootfs/sysfs
   
    # ls $CONTAINERDIR/$NAME/rootfs/dev
   

    # sudo unshare --fork --pid --uts --ipc --net --mount  chroot $CONTAINERDIR/$NAME/rootfs $INIT_CMD_ARGS
    ...
    # mount -t proc proc "$CONTAINERDIR/$NAME/rootfs/proc"
    # mount -t sysfs sysfs "$CONTAINERDIR/$NAME/rootfs/sys"
    # mount --bind /dev "$CONTAINERDIR/$NAME/rootfs/dev"
    chmod 755 "$CONTAINERDIR/$NAME/rootfs/"

    sudo unshare -p -f --net --ipc --uts --kill-child --mount-proc=$CONTAINERDIR/$NAME/rootfs/proc  chroot $CONTAINERDIR/$NAME/rootfs  $INIT_CMD_ARGS
    


    # chroot "$CONTAINERDIR/$NAME/rootfs" 
    # unshare --uts --pid --net --mount --ipc --fork --kill-child ${command:-/bin/bash}
    
    # unshare --uts --pid --net --mount --ipc  --kill-child chroot "$CONTAINERDIR/$NAME/rootfs" ${command:-/bin/bash} 
    
    #unshare --uts --pid --net --mount --ipc chroot "$CONTAINERDIR/$NAME/rootfs" "$CONTAINERDIR/$NAME/rootfs" $INIT_CMD_ARGS
}
...
exec()
{
    ...
    local CONTAINER_INIT_PID=$(pgrep -P $UNSHARE_PID | head -1)
     echo $CONTAINER_INIT_PID
    [ -z "$CONTAINER_INIT_PID" ] && die "Cannot find container process"
    
    # nsenter -t 12964  -p -u --net --mount --ipc $EXEC_CMD_ARGS
    #nsenter --target 1 --uts --pid --net --mount --ipc -w -- $EXEC_CMD_ARGS
    # Subtask 3.b.1
    ...
    # nsenter --target 1536 --uts -r --pid --net --uts --ipc --mount  --target $CONTAINER_INIT_PID $EXEC_CMD_ARGS
    sudo nsenter --pid=/proc/$CONTAINER_INIT_PID/ns/pid \
    unshare --fork --mount-proc=$CONTAINERDIR/$NAME/rootfs/proc \
    chroot $CONTAINERDIR/$NAME/rootfs $EXEC_CMD_ARGS
    
   #nsenter --target $CONTAINER_INIT_PID  --uts --pid --net --mount --ipc --wd="$CONTAINERDIR/$NAME/rootfs" $EXEC_CMD_ARGS
   #sudo nsenter --uts --pid --net --mount --ipc --wd="$CONTAINERDIR/$NAME/rootfs/proc" --target "$CONTAINER_INIT_PID" sh -c $EXEC_CMD_ARGS
   #sudo nsenter --target $CONTAINER_INIT_PID --mount --uts --ipc --net --pid $EXEC_CMD_ARGS

   #nsenter -t $CONTAINER_INIT_PID $CONTAINERDIR/$NAME/rootfs $EXEC_CMD_ARGS
}
...
addnetwork()
{
    ...
    ip link add  $OUTSIDE_PEER type veth peer name $INSIDE_PEER netns $CONDUCTORNS 
